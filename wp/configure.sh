...
for DIR in ${PROJECT_DIR} ${HTDOCS_DIR} ${LOG_DIR} ${DB_DIR} ${SOFT_DIR} ${WP_DIR}
do
    sudo install -g ${GROUP} -o ${OWNER} -d -m a+rwx,o-w,g+s ${DIR}
done
...
sudo sed -i "s/^user = www-data/user = ${OWNER}/g" /etc/php/7.2/fpm/pool.d/www.conf
sudo sed -i "s/^group = www-data/group = ${GROUP}/g" /etc/php/7.2/fpm/pool.d/www.conf

sudo sed -i "s/^listen.owner = www-data/listen.owner = ${OWNER}/g" /etc/php/7.2/fpm/pool.d/www.conf
...
cat << EOF | sudo tee /etc/nginx/sites-available/${SITENAME}.conf > /dev/null
server {
